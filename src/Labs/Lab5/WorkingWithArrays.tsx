...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <label className="float-start me-2">id:</label>
            <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
            <br/><br/>
            <label className="float-start me-2">title:</label>
            <input defaultValue={todo.title} className="form-control w-50 float-start"
                onChange={(e) => setTodo({ ...todo, title: e.target.value })} />
                <a href={`${API}/${todo.id}/title/${todo.title}`} className="btn btn-primary float-end">
                Update Todo</a>
            <br /><br />
            <label className="float-start me-2">completed:</label>
            <select className="form-select float-start w-25 me-2"
                id="boolean-select"
                value={todo.completed.toString()}
                onChange={(e) => { if (e.target.value === "true") { setTodo({ ...todo, completed: true }) } else { setTodo({ ...todo, completed: false }) } }}
            >
            ...
            <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
                Update Todo</a>
            <br /><br />
            <label className="float-start me-2">description:</label>
